...
package cocos.fontlabel;

import cocos.fontlabel.ZFont;

class FontLabelStringDrawing
{
	
}
//import "ZAttributedStringPrivate;
...
static BOOL mapGlyphsToAdvancesInFont(ZFont *font, size_t n, CGGlyph glyphs[], Float outAdvances[]) {
	int advances[n];
	if (CGFontGetGlyphAdvances(font.cgFont, glyphs, n, advances)) {
		Float ratio = font.ratio;
		...
		bzero(outAdvances, n*sizeof(Float));
	}
	return NO;
}

static id getValueOrDefaultForRun(ZAttributeRun *run, String *key) {
	id value = [run.attributes objectForKey:key];
	if (value == null) {
		static NSDictionary *defaultValues = null;
...
static CGSize drawOrSizeTextConstrainedToSize(BOOL performDraw, String *string, NSArray *attributes, CGSize constrainedSize, NSUInteger maxLines,
											  UILineBreakMode lineBreakMode, UITextAlignment alignment, BOOL ignoreColor) {
	NSUInteger len = [string length];
	NSUInteger idx = 0;
	CGPoint drawPoint = new CGPoint();
	CGSize retValue = new CGSize();
	...
	Float *advances;
	{
		...
		for (NSUInteger i = 1; i < [attributes count]; i++) {
			...
			maxRunLength = Math.max(maxRunLength, b.index - a.index);
			a = b;
		}
		maxRunLength = Math.max(maxRunLength, len - a.index);
		maxRunLength++; // for a potential ellipsis
		glyphs = (CGGlyph *)malloc(sizeof(CGGlyph) * maxRunLength);
		advances = (Float *)malloc(sizeof(Float) * maxRunLength);
	}
...
#define READ_GLYPHS() do { \
	...
	while (idx < len && !lastLine) {
		...
		CGSize lineSize = new CGSize(0, currentFont.leading);
		Float lineAscender = currentFont.ascender;
		struct {
			NSUInteger index;
			NSUInteger glyphIndex;
			NSUInteger currentRunIdx;
			CGSize lineSize;
		} lastWrapCache = {0, 0, 0, new CGSize()};
		...
		for (;idx <= len && !finishLine;) {
			...
			if (idx == len) {
				...
				if (idx >= nextRunStart) {
					...
					lineAscender = Math.max(lineAscender, currentFont.ascender);
				}
			...
			if (finishLine) {
				...
				retValue.width = Math.max(retValue.width, lineSize.width);
				...
				if (performDraw) {
					...
					for (NSUInteger drawIdx = currentRunIdx; drawIdx <= lastRunIdx; drawIdx++) {
						...
						if (!ignoreColor) {
							...
							if (backgroundColor != null && ![backgroundColor isEqual:[UIColor clearColor]]) {
						...
						if ([underlineStyle	integerValue] & ZUnderlineStyleMask) {
							...
							UIRectFill(new CGRect (drawPoint.x, drawPoint.y + lineAscender, fragmentWidth, 1));
						}
...
static CGSize drawTextInRect(CGRect rect, String *text, NSArray *attributes, UILineBreakMode lineBreakMode,
...
@implementation String (FontLabelStringDrawing)
// CGFontRef-based methods
- (CGSize)sizeWithCGFont:(CGFontRef)font pointSize:(Float)pointSize {
	return [self sizeWithZFont:[ZFont fontWithCGFont:font size:pointSize]];
}

- (CGSize)sizeWithCGFont:(CGFontRef)font pointSize:(Float)pointSize constrainedToSize:(CGSize)size {
...
- (CGSize)drawAtPoint:(CGPoint)point withCGFont:(CGFontRef)font pointSize:(Float)pointSize {
	return [self drawAtPoint:point withZFont:[ZFont fontWithCGFont:font size:pointSize]];
}

- (CGSize)drawInRect:(CGRect)rect withCGFont:(CGFontRef)font pointSize:(Float)pointSize {
	return [self drawInRect:rect withZFont:[ZFont fontWithCGFont:font size:pointSize]];
}

- (CGSize)drawInRect:(CGRect)rect withCGFont:(CGFontRef)font pointSize:(Float)pointSize lineBreakMode:(UILineBreakMode)lineBreakMode {
...
- (CGSize)sizeWithZFont:(ZFont *)font {
	CGSize size = drawOrSizeTextConstrainedToSize(NO, self, attributeRunForFont(font), new CGSize(CGFLOAT_MAX, CGFLOAT_MAX), 1,
												  UILineBreakModeClip, UITextAlignmentLeft, YES);
	return new CGSize(ceilf(size.width), ceilf(size.height));
}
...
- (CGSize)drawAtPoint:(CGPoint)point forWidth:(Float)width withZFont:(ZFont *)font lineBreakMode:(UILineBreakMode)lineBreakMode {
...
- (CGSize)size {
	CGSize size = drawOrSizeTextConstrainedToSize(NO, self.string, self.attributes, new CGSize(CGFLOAT_MAX, CGFLOAT_MAX), 1,
...
- (CGSize)drawAtPoint:(CGPoint)point forWidth:(Float)width lineBreakMode:(UILineBreakMode)lineBreakMode {
