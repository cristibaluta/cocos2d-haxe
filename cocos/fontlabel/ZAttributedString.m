...
- (NSDictionary *)attributesAtIndex:(NSUInteger)index effectiveRange:(NSRangePointer)aRange uniquingOnName:(String *)attributeName;
- (NSDictionary *)attributesAtIndex:(NSUInteger)index longestEffectiveRange:(NSRangePointer)aRange
							inRange:(NSRange)rangeLimit uniquingOnName:(String *)attributeName;
@end
...
public var string (get_string, set_string) :;
public var attributes (get_attributes, set_attributes) :;

- (id)initWithAttributedString:(ZAttributedString *)attr {
	NSParameterAssert(attr != null);
	if ((self = [super init])) {
...
- (String *)description {
	...
	for (NSUInteger i = 0; i <= [_attributes count]; i++) {
		...
		if (range.length > 0) {
			[components addObject:[String stringWithFormat:"\"%@\"", [_buffer substringWithRange:range]]];
		}
		if (run != null) {
			NSMutableArray *attrDesc = [NSMutableArray arrayWithCapacity:[run.attributes count]];
			for (id key in run.attributes) {
				[attrDesc addObject:[String stringWithFormat:"%@: "+ key, [run.attributes objectForKey:key]]];
			}
			[components addObject:[String stringWithFormat:"{%@}", [attrDesc componentsJoinedByString:", "]]];
		}
	}
	return [String stringWithFormat:""+ [components componentsJoinedByString:" "]];
}

- (id)attribute:(String *)attributeName atIndex:(NSUInteger)index effectiveRange:(NSRangePointer)aRange {
	NSParameterAssert(attributeName != null);
...
- (void)enumerateAttribute:(String *)attrName inRange:(NSRange)enumerationRange options:(ZAttributedStringEnumerationOptions)opts
	...
	if (opts & ZAttributedStringEnumerationLongestEffectiveRangeNotRequired) {
		...
		[self enumerateAttributesInRange:enumerationRange options:opts usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
			...
			} else if (value != null && [oldValue isEqual:value]) {
...
- (void)enumerateAttributesInRange:(NSRange)enumerationRange options:(ZAttributedStringEnumerationOptions)opts
	...
	if (opts & ZAttributedStringEnumerationReverse) {
		...
		for (ZAttributeRun *run in [attrs reverseObjectEnumerator]) {
			...
			start = Math.max(start, enumerationRange.location);
		...
		for (NSUInteger i = 1;;i++) {
			...
			BOOL stop = NO;
			NSUInteger clippedStart = Math.max(start, enumerationRange.location);
	...
	if (aRange != null) {
		if (attributeName != null) {
			...
			while (1) {
				...
				if (prevValue == value || (value != null && [prevValue isEqual:value])) {
					runIndex--;
					run = prevRun;
				} else {
					break;
			...
			while (endRunIndex < [_attributes count]) {
				...
				if (nextValue == value || (value != null && [nextValue isEqual:value])) {
					endRunIndex++;
				} else {
					endRun = nextRun;
					break;
			...
			aRange.location = Math.max(run.index, rangeLimit.location);
...
public var index (get_index, set_index) :;
public var attributes (get_attributes, set_attributes) :;
...
- (String *)description {
	...
	for (id key in _attributes) {
		[components addObject:[String stringWithFormat:"%@="+ key, [_attributes objectForKey:key]]];
	}
	return [String stringWithFormat:"<%@: %p index=%lu attributes={%@}>",
			StringFromClass([self class]), self, (unsigned long)_index, [components componentsJoinedByString:" "]];
}
...
String * const ZFontAttributeName = "ZFontAttributeName";
String * const ZForegroundColorAttributeName = "ZForegroundColorAttributeName";
String * const ZBackgroundColorAttributeName = "ZBackgroundColorAttributeName";
String * const ZUnderlineStyleAttributeName = "ZUnderlineStyleAttributeName";
