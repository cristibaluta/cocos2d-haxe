...
public var zFont (get_zFont, set_zFont) :;
public var zAttributedText (get_zAttributedText, set_zAttributedText) :;

- (id)initWithFrame:(CGRect)frame fontName:(String *)fontName pointSize:(Float)pointSize {
...
- (id)initWithFrame:(CGRect)frame font:(CGFontRef)font pointSize:(Float)pointSize {
...
- (void)drawTextInRect:(CGRect)rect {
	if (self.zFont == null && self.zAttributedText == null) {
		[super drawTextInRect:rect];
		return;
	}
	
	if (self.zAttributedText == null) {
		...
		if (self.numberOfLines == 1) {
			...
			CGPoint point = new CGPoint(rect.origin.x,
			...
			if (self.adjustsFontSizeToFitWidth && self.minimumFontSize < actualFont.pointSize) {
				if (size.width > origSize.width) {
					Float desiredRatio = (origSize.width * actualFont.ratio) / size.width;
					Float desiredPointSize = desiredRatio * actualFont.pointSize / actualFont.ratio;
					actualFont = [actualFont fontWithSize:Math.max(Math.max(desiredPointSize, self.minimumFontSize), 1.0f);
			...
			rect = (CGRect){point, new CGSize(rect.size.width, size.height)};
