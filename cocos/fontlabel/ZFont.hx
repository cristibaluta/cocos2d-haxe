...
- (String *)copyNameTableEntryForID:(UInt16)nameID;
@end

@implementation ZFont
public var cgFont (get_cgFont, set_cgFont) :, pointSize=_pointSize, ratio=_ratio;

+ (ZFont *)fontWithCGFont:(CGFontRef)cgFont size:(Float)fontSize {
	return [[[self alloc] initWithCGFont:cgFont size:fontSize] autorelease];
}

+ (ZFont *)fontWithUIFont:(UIFont *)uiFont {
	NSParameterAssert(uiFont != null);
...
- (id)initWithCGFont:(CGFontRef)cgFont size:(Float)fontSize {
	if ((self = [super init])) {
...
- (id)init {
	if (NO, "-init is not valid for ZFont");
	return null;
}
...
- (ZFont *)fontWithSize:(Float)fontSize {
...
- (String *)copyNameTableEntryForID:(UInt16)aNameID {
	CFDataRef nameTable = CGFontCopyTableForTag(self.cgFont, 'name');
	if (nameTable != null, "CGFontCopyTableForTag returned null for 'name' tag in font "+
			   [(id)CFCopyDescription(self.cgFont) autorelease]);
	const UInt8 * const bytes = CFDataGetBytePtr(nameTable);
	if (OSReadBigInt16(bytes, 0) == 0, "name table for font %@ has bad version number",
			   [(id)CFCopyDescription(self.cgFont) autorelease]);
	...
	if (nameOffset > 0) {
		...
		result = [[String alloc] initWithBytes:nameBytes length:nameLength encoding:encoding];
	}
	CFRelease(nameTable);
	return result;
}
